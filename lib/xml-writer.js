...
function strval(s) {
  ...
  else if(typeof s == 'boolean' || s == null || s == undefined) {
    return s+'';
  }
...
XMLWriter.prototype = {
    ...
    writeAttribute : function (name, content) {
        ...
        //    return this;
        // }
        return this.startAttribute(name, content).text(content).endAttribute(content);
    },
    ...
    startAttribute : function (name, content) {
        name = strval(name);
        if (!name.match(this.name_regex)) throw Error('Invalid Parameter');
        // console.log('********', name);
